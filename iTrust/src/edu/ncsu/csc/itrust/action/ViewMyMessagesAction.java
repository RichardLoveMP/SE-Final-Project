...
public class ViewMyMessagesAction {
	...
	/**
	 * Gets all the messages for the logged in user and sorts by descending time
	 *
	 * @return a list of all the user's messages
	 * @throws SQLException
	 * @throws DBException
	 */
	public List<MessageBean> getAllMyMessagesTimeDescending() throws SQLException, DBException {
	 ...
	 */
	public List<MessageBean> filterMessages(List<MessageBean> messages, String filter) throws ITrustException, ParseException, ArrayIndexOutOfBoundsException {
		List<MessageBean> filtered = new ArrayList<MessageBean>();

		String[] f = filter.split(",", -1);

		String filterSender = f[0];
		String filterSubject = f[1];
		String filterHasWords = f[2];
		String filterHasNoWords = f[3];
		String filterStartDate = f[4];
		String filterEndDate = f[5];

		for(MessageBean m : messages) {
			// TODO: Code smells detected here! Bad variable name! Waiting to be fixed.
			/*
	...
	public void setUserMessageFilter(String filter, long mid) throws DBException, SQLException{
		messageDAO.setMessageFilter(filter, mid);
	}

	public String getUserMessageFilter(long mid) throws DBException{
		return messageDAO.getMessageFilter(mid);
	}

	public String[] getUserMessageFilterAsList(long mid) throws DBException {
		String[] dbFilter = messageDAO.getMessageFilter(mid).split(",");
		if (dbFilter.length == 6)
			return dbFilter;

		String[] toReturn = new String[6];
		if (dbFilter.length != 6) {
			for (int i = 0; i < dbFilter.length; i++)
				toReturn[i] = dbFilter[i];
			for (int i = dbFilter.length; i < 6; i++) {
