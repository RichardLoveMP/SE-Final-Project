...
public class TransactionDAO {
	...
	/**
	 * Returns a specific transaction log
	 *
	 * @return
	 * @throws DBException
	 */
	public List<TransactionBean> getTransactionsAccordingToID(long ID) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
	...
	//TODO:
	public List<TransactionBean> getFilteredAnonymizedTransactions(String loggedInRole, String secondaryRole, Date startDate, Date endDate, String transactionCode) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;

		String sql = "";
		sql += "SELECT loggedIn.role AS loggedInRole, secondary.role AS secondaryRole, transactionlog.transactionCode, transactionlog.addedInfo, transactionlog.timeLogged ";
		sql += "FROM transactionlog ";
		sql += "LEFT JOIN users loggedIn ON loggedIn.MID = transactionlog.loggedInMID ";
		sql += "LEFT JOIN users secondary ON secondary.MID = transactionlog.secondaryMID ";
		sql += "WHERE loggedIn.role LIKE ? ";
		sql += "AND COALESCE(secondary.role, '') LIKE ? ";
		sql += "AND transactionlog.timeLogged BETWEEN ? AND ? ";
		sql += "AND transactionlog.transactionCode LIKE ? ";
		sql += "ORDER BY transactionlog.timeLogged DESC";
		...
		try {
			...
			ps.setTimestamp(4, new Timestamp(endDate.getTime() + 1000L * 60L * 60 * 24L));
			ps.setString(5, transactionCode);
			ResultSet rs = ps.executeQuery();
			List<TransactionBean> anonymizedTransactions = loader.loadList(rs);
			rs.close();
