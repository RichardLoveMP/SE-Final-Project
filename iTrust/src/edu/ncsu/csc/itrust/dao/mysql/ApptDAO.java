...
/**
 * Helper function to get appointments in a certain period of times.
 * Used in SendReminderAction.
 * you have to make sure finalDate is after current time...
 * */

	public List<ApptBean> getApptsInCertainDays(String time) throws Exception {
		Connection conn = null;
		PreparedStatement pstring = null;
		...
		Date finalTime = dateFormat.parse(time);
		if (currentTime.after(finalTime)){
			throw new Exception("You have to give a time after now!");
		}
		...
		try{
			...
			pstring = conn.prepareStatement("SELECT * FROM appointment WHERE sched_date <= ? AND sched_date > NOW() ORDER BY sched_date;");

			Timestamp temp = new Timestamp(finalTime.getTime());
			pstring.setTimestamp(1, temp);
