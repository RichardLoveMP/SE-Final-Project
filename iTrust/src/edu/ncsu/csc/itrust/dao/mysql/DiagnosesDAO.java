...
public class DiagnosesDAO {
	...
	public DiagnosisStatisticsBean getDiagnosisCounts(String icdCode, String zipCode, java.util.Date lower, java.util.Date upper) throws DBException {
		...
		try {
			...
			int local = rs.getRow();
			ps.close();
			ps = conn.prepareStatement("SELECT * FROM ovdiagnosis INNER JOIN officevisits ON ovdiagnosis.VisitID=officevisits.ID " +
					"INNER JOIN patients ON officevisits.PatientID=patients.MID WHERE ICDCode=? AND zip LIKE ? AND visitDate >= ? AND visitDate <= ? ");
			ps.setString(1, icdCode);
			...
			ps = conn.prepareStatement("SELECT * FROM ovdiagnosis INNER JOIN officevisits ON ovdiagnosis.VisitID=officevisits.ID " +
					"INNER JOIN patients ON officevisits.PatientID=patients.MID WHERE ICDCode=? AND visitDate >= ? AND visitDate <= ? ");
			ps.setString(1, icdCode);
			...
			dsBean = new DiagnosisStatisticsBean(zipCode, local, region, state, all, lower, upper);
			rs.close();
			return dsBean;
		} catch (SQLException e) {
