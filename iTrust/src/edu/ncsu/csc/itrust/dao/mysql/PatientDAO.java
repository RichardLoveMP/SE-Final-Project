...
public class PatientDAO {
	...
	// /**
	//  * Adds a pre patient to the table, returns the new MID
	//  * 
	//  * @return The MID of the patient as a long.
	//  * @throws DBException
	//  */
	public void updateNewPatient(PatientBean p) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("UPDATE patients SET firstName=?,lastName=?,email=?,"
					...
					+ "AlternateName=?, DateOfDeactivation=?, isRegistered=?, pAddress=? WHERE MID=?");
	 ...
	 * @throws DBException
	 */
	public long addPrePatient(PatientBean prePatient) throws DBException {
		String firstName = prePatient.getFirstName();
		String lastName = prePatient.getLastName();
		String email = prePatient.getEmail();
		String phone = prePatient.getPhone();
		String address = prePatient.getStreetAddress1();
		int isRegistered = prePatient.getIsRegistered();
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement(
					"INSERT INTO patients(firstName, lastName, email, phone, address1, isRegistered) VALUES(?, ?, ?, ?, ?, ?)");
			ps.setString(1, firstName);
			ps.setString(2, lastName);
			ps.setString(3, email);
			ps.setString(4, phone);
			ps.setString(5, address);
			ps.setInt(6, isRegistered);
			ps.executeUpdate();
			long mid = DBUtil.getLastInsert(conn);
			ps.close();
			return mid;
		} catch (SQLException e) {
			throw new DBException(e);
	...
	public int isRegistered(long mid) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		int ret = 1;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT isRegistered FROM patients WHERE MID=?;");
			ps.setLong(1, mid);
			ResultSet rs = ps.executeQuery();

			if (rs.next()) {
				ret = rs.getInt("isRegistered");
			}
	...
	public void addHistory(long pid, long hcpid) throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement(
					"INSERT INTO historypatients SELECT null, CURDATE(), ?, p.* FROM patients p WHERE p.mid=?");
			ps.setLong(1, hcpid);
			ps.setLong(2, pid);
			ps.executeUpdate();
			ps.close();
	...
	//UC92
	/**
	 * Updates removes a patient with the given MID
	 * 
	 * @param mid mid of the patient to delete
	 * @throws DBException
	 */
	public boolean deletePatient(long pid) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
	...
	public List<PrescriptionBean> getCurrentPrescriptions(long patientID) throws DBException {
		...
		try {
			...
			ps = conn.prepareStatement("Select * From ovmedication,ndcodes,officevisits "
					+ "Where officevisits.PatientID = ? AND ovmedication.VisitID = "
					+ "officevisits.ID AND ndcodes.Code=ovmedication.NDCode AND " + "ovmedication.EndDate >= ?"
					+ "ORDER BY ovmedication.ID DESC;");
			ps.setLong(1, patientID);
	...
	/**
	 * Lists every pre-patient in the database.
	 * 
	 * @return A java.util.List of PatientBeans representing the patients.
	 * @throws DBException
	 */
	public List<PatientBean> getAllPrePatients() throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT * FROM patients WHERE isRegistered=0");
			ResultSet rs = ps.executeQuery();
			List<PatientBean> loadlist = patientLoader.loadList(rs);
			rs.close();
			ps.close();
	...
	public List<PatientBean> getRenewalNeedsPatients(long hcpMID) throws DBException {
		...
		try {
			...
			ps = conn.prepareStatement("SELECT * FROM ( " +
					...
					"declaredhcp.HCPID = ? AND " + "patients.MID = declaredhcp.PatientID AND " +
	...
	/**
	 * Returns all patients with names "LIKE" with wildcards (as in SQL) the passed
	 * in parameters.
	 * 
	 * @param first The patient's first name.
	 * @param last  The patient's last name.
	...
	public int addDesignatedNutritionist(long patientMID, long HCPID) throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("INSERT INTO " + "designatedNutritionist(PatientID, HCPID) VALUES(?,?);");
	...
	/**
	 * Returns the ID of the designated nutritionist for the patient returns -1 if
	 * the patient does not have a designated nutritionist
	 */
	public long getDesignatedNutritionist(long patientMID) throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT HCPID FROM " + "designatedNutritionist WHERE PatientID = ?;");
			ps.setLong(1, patientMID);
	...
	public int updateDesignatedNutritionist(long patientMID, long HCPID) throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("UPDATE designatedNutritionist " + "SET HCPID = ? WHERE PatientID = ?;");
			ps.setLong(1, HCPID);
	...
	public int deleteDesignatedNutritionist(long patientMID) throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("DELETE FROM designatedNutritionist " + "WHERE PatientID = ?;");
			ps.setLong(1, patientMID);
	...
	public int getEmailCount(String email) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		int count = 0;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT count(*) as cnt FROM patients WHERE email=?");
	...
	public void setPharmacy(long pid, String pharmAddr) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("UPDATE patients SET pAddress=? WHERE MID=?");
		        ps.setString(1, pharmAddr);
			ps.setLong(2, pid);
	...
	public String getPharmacy(long pid) throws ITrustException, DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			 ps = conn.prepareStatement("SELECT pAddress FROM patients WHERE MID=?");
			 ps.setLong(1, pid);
			 ResultSet rs;
			 rs = ps.executeQuery();
			 if (rs.next()) {
				 String result = rs.getString("pAddress");
				 rs.close();
				 ps.close();
				 return result;
			 } else {
