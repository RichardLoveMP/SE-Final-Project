...
public class MessageDAO {
	...
	/**
	 * Gets all the messages for a certain user MID sorted by descending time.
	 * @param mid The MID of the user to be looked up.
	 * @return A java.util.List of MessageBeans.
	 * @throws SQLException
	 * @throws DBException
	 ...
	 * @throws SQLException
	 */
	public List<MessageBean> getMessagesFromTimeDescending(long mid) throws DBException, SQLException {
		Connection conn = null;
                PreparedStatement ps = null;
                try{
                conn = factory.getConnection();
	...
	public void setMessageFilter(String filter, long mid) throws SQLException, DBException{
		Connection conn = null;
                PreparedStatement ps = null;
		try{
			conn = factory.getConnection();
			ps = conn.prepareStatement("UPDATE users SET messageFilter=? WHERE MID=?");
	...
	public String getMessageFilter(long mid) throws DBException{
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT messageFilter FROM users WHERE MID=?");
			ps.setLong(1, mid);
			final ResultSet results = ps.executeQuery();
			if (results.next()) {
				final String result = results.getString("messageFilter");
				results.close();
				ps.close();
				return result;
			} else {
