...
String currPharmacy = patientAction.getPharmacy();
...
if (request.getParameter("pharmacy") != null) {
	String newPharmacy = "";
	newPharmacy = request.getParameter("pharmacy");
	patientAction.setPharmacy(newPharmacy);
	currPharmacy = newPharmacy;
}

if (request.getParameter("addprescription") != null) {
	String prescriptionName = request.getParameter("medID");
	long pid = Long.parseLong(pidString);
	String addInfo = prescriptionName + " sent to pharmacy " + currPharmacy;
	loggingAction.logEvent(TransactionType.HCP_SENT_PRESCRIPTION, loggedInMID, pid, addInfo);
}
...
<table class="fTable" align="center" id="prescriptionsTable">
    <tr>
        <th colspan="7"><a href="#" class="topLink">[Top]</a>Prescriptions</th>
    </tr>
    <tr class="subHeader" colspan="7">
        <td>Medication</td>
        <td>Dosage</td>
        <td>Dates</td>
    ...
    <%if (ovaction.prescriptions().getPrescriptions().size()==0){ %>
        <tr>
            <td colspan="7" style="text-align: center;">No Prescriptions on record</td>
        </tr>
    <%} else { %>
    ...
    <tr>
        <th colspan="7" style="text-align: center;">Add New</th>
    </tr>
     <tr>
        <td align=center>
	...
	<td align=center>
	    <input type="text" name="pharmacy" id"pharmacy"
	    	<% if (currPharmacy.equals("")) {%>
	    		value="Enter Pharmacy Here"
		<% } else if (currPharmacy != null) {%> 
	    		value="<%= StringEscapeUtils.escapeHtml(currPharmacy) %>"
		<% } %>	
	    >
	</td>
	<td>
